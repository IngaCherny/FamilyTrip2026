          ...
          return (
            <article key={c.id} className="card-paper flex flex-col p-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-stone-500">{tc(regionById(c.region).name)}</span>
              </div>

              <h3 className="mt-2 font-serif text-xl font-bold text-stone-900">{tc(c.name)}</h3>
              <p className="mt-1 text-sm text-stone-600">{tc(c.summary)}</p>
              ...
              <p className="mt-3 rounded-lg bg-meadow-50 p-2.5 text-sm text-meadow-700 ring-1 ring-meadow-100">
                <span className="font-semibold">{tc("Do this")}: </span>
                {tc(c.action)}
              </p>

              {c.caveat && <p className="mt-2 text-xs text-stone-500">{tc(c.caveat)}</p>}
