  ...
  const body = (
    <div className="space-y-3 px-3 pb-3">
      ...
      {officialLink && (
        ...
        >
          {link ? tc(linkLabel ?? "Official route") : tc("Official site")}
        </a>
      )}
            ...
            <div className="mb-1 flex flex-wrap items-center gap-1.5">
              ...
              {(shutToday || offSeason) && (
                <span className="rounded-full bg-sunset-300 px-2 py-0.5 text-[11px] font-semibold text-stone-900">
                  {shutToday ? tc("Closed today") : tc("Out of season")}
                </span>
              )}
            </div>
      ...
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-3 p-2.5 text-left">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs">
            {tag && <span className={onDark ? "text-[#EDE8DC]/60" : "text-stone-500"}>{tc(TAG_META[tag].label)}</span>}
            {total && <span className={`font-medium ${onDark ? "text-[#D9A441]" : "text-meadow-700"}`}>{total}</span>}
            {attraction?.buggy && <span className={onDark ? "text-[#EDE8DC]/60" : "text-meadow-600"}>{tc("Buggy OK")}</span>}
            {(shutToday || offSeason) && (
  ...
  const tileOverlay = (
    <>
      ...
      <div className="absolute inset-x-0 top-0 flex items-center justify-between p-4">
        ...
        {isToday && (
          <span className="rounded-full bg-glacier-600 px-2.5 py-1 text-[11px] font-bold text-white">{tc("Today")}</span>
        )}
      </div>
                ...
                <div className="order-2 rounded-xl bg-white/[0.06] p-3 ring-1 ring-white/10">
                  ...
                  {day.drive.stops && day.drive.stops.length > 0 && (
                    <div className="mt-3 space-y-2">
                      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#D9A441]">{tc("Stops along the way")}</p>
                        ...
                        <>
                          ...
                          >
                            <span className="text-sm font-medium text-[#EDE8DC]/80">
                              {showStops
                                ? tc("Hide extra stops")
                                : `${day.drive.stops.length - 1} ${tc(day.drive.stops.length - 1 > 1 ? "more stops" : "more stop")}`}
                            </span>
                      ...
                      {showChange && (
                        <div className="mt-2 space-y-1.5">
                          ...
                          <p className="pt-1 text-xs text-[#EDE8DC]/50">
                            {tc("Every option also lives under Places, with its own map and “near me”.")}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="pt-2">
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        >
                          ...
                          <span className="shrink-0 text-xs font-semibold text-[#D9A441]">{tc("on Maps →")}</span>
                        </a>
                        {nearby.map(({ a, km }) => (
                          <a
                            key={a.id}
